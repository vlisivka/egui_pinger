...
/// Renders the host settings window and returns whether the help button was clicked.
pub fn render_host_settings_window(
    ...
    let Some(ref addr) = editing_host.clone() else {
        return false;
    };

    let mut is_open = true;
    let host_copy = hosts.iter().find(|h| h.address == *addr).cloned();
    ...
    if let Some(mut h) = host_copy {
        ...
        // Save changes back to host list
        if let Some(target) = hosts.iter_mut().find(|th| th.address == *addr) {
            *target = h;
        }
    }

    // Close window if "Close" button was clicked
    // or the 'x' in the window title bar was pressed
    if !is_open {
        *editing_host = None;
    }
