        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                egui::ScrollArea::horizontal().show(ui, |ui| {
                    ...
                    // Модальне вікно трасування
                    if let Some(ref addr) = self.viewing_route.clone() {
                        let mut open = true;
                                    ...
                                    if let Some(status) = state.statuses.get(addr) {
                                        if status.traceroute_path.is_empty() {
                                            ...
                                            for (i, hop_addr) in status.traceroute_path.iter().enumerate() {
                                                let hop_status = state.statuses.get(hop_addr);
                                                ui.horizontal(|ui| {
                                                    ui.label(format!("{}.", i + 1));
                                                    ui.monospace(hop_addr);
                                                    if let Some(hs) = hop_status {
                                                        if hs.alive {
                                                            ui.label(RichText::new(format!("({:.1} ms)", hs.latency)).color(visuals.latency_color(hs.latency)));
                                                        } else {
                                                            ui.label(RichText::new(tr!("(TIMEOUT)")).color(Color32::from_rgb(213, 94, 0)));
                                                        }
                                                    }
                                                });
                                            }
